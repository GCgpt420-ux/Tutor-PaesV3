...
export default function ResultadosGlobalPage() {
  ...
  if (!attemptId) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] p-8 text-center animate-in fade-in slide-in-from-bottom-4">
        <h2 className="text-2xl font-black text-text-primary uppercase tracking-tight mb-3">No se detecto el intento</h2>
        <p className="text-text-tertiary font-medium mb-8">La secuencia de resultados no esta disponible para esta sesion.</p>
        <button
          onClick={() => router.back()}
  ...
  return (
    <div className="min-h-screen py-10 bg-surface-base">
      <ExamResultsView attemptId={attemptId} />
    </div>
  );
}
